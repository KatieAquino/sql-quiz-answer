...
SELECT id FROM orders WHERE orders.customer_id = (SELECT customers.id
FROM customers WHERE fname = 'Jane' AND lname = 'Doe');
...
SELECT cupcakes.name, SUM(num_cupcakes) FROM cupcakes LEFT JOIN orders
ON cupcake_id = cupcakes.id
...
SELECT email FROM customers JOIN orders ON customers.id = customer_id
GROUP BY email
HAVING SUM(orders.num_cupcakes) > 100;
...
WHERE orders.customer_id = (SELECT orders.customer_id FROM cupcakes JOIN orders
ON cupcakes.id = orders.cupcake_id
 WHERE orders.processed = True AND cupcakes.name = 'funfetti'
